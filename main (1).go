...
type FileRecord struct {
	...
	Description   string     `bson:"description,omitempty" json:"description,omitempty"` // [NEW] per-variant title/description text (e.g. "Movie — Hindi audio")
}
...
func (db *DB) saveFile(ctx context.Context, f *FileRecord) error {
	...
	update := bson.M{
		"$set": bson.M{
			...
			"description":    f.Description, // [NEW] persist per-variant description alongside title(file_name)
		},
	}
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "processlang":
		...
		if rec.LangGroupID != nil && *rec.LangGroupID != "" {
			// Don't just trust the flag — verify siblings actually exist.
			// A file could be stuck "tagged" from a run where extraction
			// failed for every track (a bug in an earlier version of this
			// feature). In that case, treat it as unprocessed and retry.
...
// probeAudioTracks runs ffprobe on a local file and returns its audio
// streams in order. Requires ffprobe to be present in the container image.
func probeAudioTracks(ctx context.Context, path string) ([]audioTrackInfo, error) {
...
// extractAudioVariant remuxes inputPath into outputPath keeping the video
// stream and exactly one audio stream (audioStreamIndex, 0-based among audio
// streams only).
//
// [UPDATED] Previously this did a full "-c copy" (video AND audio untouched,
// same container as source, e.g. .mkv) — fast but not reliably playable in
// browsers, since the source audio codec (AC3/DTS/etc.) often isn't
// browser-supported. Now: video is still stream-copied (-c:v copy, no
// re-encode, so this stays fast), but audio is re-encoded to AAC 128k
// (-c:a aac -b:a 128k) and output is forced to MP4 with -movflags
// +faststart (moov atom moved to the front so playback can start before the
// whole file downloads). outputPath must now end in .mp4.
func extractAudioVariant(ctx context.Context, inputPath, outputPath string, audioStreamIndex int) error {
...
// downloadFullFileToTemp pulls the entire Telegram file down to a local temp
// path so ffprobe/ffmpeg (which need random-access files, not an HTTP range
// stream) can work on it.
func (a *App) downloadFullFileToTemp(ctx context.Context, channelID int64, messageID int, fileSize int64, destPath string) error {
	location, tgSize, err := a.mtPool.getFileLocation(ctx, channelID, messageID)
	if err != nil {
		return fmt.Errorf("getFileLocation: %w", err)
	}
	...
	reader := newTgFileReader(ctx, api, a.cfg, location, fileSize, 0, fileSize-1)
	defer reader.Close()
...
// the file has more than one embedded audio track, it extracts every extra
// track into its own standalone file, uploads each to the storage channel,
// and saves it as a FileRecord linked via LangGroupID so the watch page can
// offer a language switcher. Errors are logged, not surfaced to the user —
// the original file and link already work fine without this.
func (a *App) processAudioLanguages(chatID int64, notifyUserID int64, slug string, messageID int, channelID int64, fileName string, fileSize int64) {
	...
	tmpDir, err := os.MkdirTemp("", "rajlang-*")
	if err != nil {
		a.logger.Warn("processAudioLanguages: mkdtemp failed", zap.Error(err))
		if notifyUserID != 0 {
			a.pool.send(chatID, fmt.Sprintf("❌ `%s` — server temp folder error, dobara try karo.", slug))
		}
		return
	}
	defer os.RemoveAll(tmpDir)

	srcPath := filepath.Join(tmpDir, "src"+filepath.Ext(fileName))
	if err := a.downloadFullFileToTemp(ctx, channelID, messageID, fileSize, srcPath); err != nil {
		a.logger.Warn("processAudioLanguages: download failed", zap.Error(err), zap.String("slug", slug))
		if notifyUserID != 0 {
			a.pool.send(chatID, fmt.Sprintf("❌ `%s` — Telegram se file download nahi ho payi, dobara try karo.", slug))
		}
		return
	}

	tracks, err := probeAudioTracks(ctx, srcPath)
	if err != nil {
		a.logger.Warn("processAudioLanguages: probe failed", zap.Error(err), zap.String("slug", slug))
		if notifyUserID != 0 {
			a.pool.send(chatID, fmt.Sprintf("❌ `%s` — ffprobe fail ho gaya, ffmpeg image mein sahi se installed hai check karo.", slug))
		}
		return
	}
	if len(tracks) < 2 {
		if notifyUserID != 0 {
			a.pool.send(chatID, fmt.Sprintf("ℹ️ `%s` mein sirf ek hi audio track mila — language switcher ki zaroorat nahi.", slug))
		}
		return // nothing to do — single (or no) audio track
	}

	langGroupID := uuid.New().String()
	base := filepath.Base(fileName)
	ext := filepath.Ext(base)
	nameNoExt := strings.TrimSuffix(base, ext)

	// [UPDATED] Was ISO-tag-derived labels (langLabel) covering every extra
	// track found. New simplified requirement: exactly two fixed outputs —
	// track 0 is always treated as Hindi, track 1 is always treated as
	// English. Any tracks beyond the first two are ignored.
	fixedLabels := []string{"Hindi", "English"}

	var createdLabels []string
	var lastErr error

	// [UPDATED] Was `tracks[1:]` — it skipped the first/default audio track
	// entirely and only extracted extra ones (relying on the original raw
	// MKV to serve as the first-track variant). Now both of the first two
	// tracks are explicitly extracted into their own standalone,
	// browser-ready MP4s (hindi.mp4 / english.mp4), since the original MKV's
	// audio isn't guaranteed to play correctly in a browser.
	for i, track := range tracks[:2] {
		label := fixedLabels[i]
		outPath := filepath.Join(tmpDir, fmt.Sprintf("variant_%d.mp4", i))

		if err := extractAudioVariant(ctx, srcPath, outPath, track.StreamIndex); err != nil {
			a.logger.Warn("processAudioLanguages: extract failed", zap.Error(err), zap.String("slug", slug), zap.String("lang", label))
			lastErr = err
			continue
		}
		...
		if err != nil {
			a.logger.Warn("processAudioLanguages: reopen variant failed", zap.Error(err))
			lastErr = err
			continue
		}
		outName := fmt.Sprintf("%s [%s].mp4", nameNoExt, label) // [UPDATED] always .mp4 now, was same ext as source
		fileBytes := tgbotapi.FileReader{Name: outName, Reader: f}
		doc := tgbotapi.NewDocument(a.cfg.DBChannelID, fileBytes)
		sentMsg, sendErr := a.pool.next().Send(doc)
		f.Close()
		if sendErr != nil {
			a.logger.Warn("processAudioLanguages: upload failed", zap.Error(sendErr), zap.String("lang", label))
			lastErr = sendErr
			continue
		}
		...
		variantSlug := uuid.New().String()
		variantRec := &FileRecord{
			ID:           variantSlug,
			MessageID:    sentMsg.MessageID,
			ChannelID:    toInternalChannelID(a.cfg.DBChannelID),
			FileName:     outName,                                        // title (existing field, already used as title everywhere in this codebase)
			FileSize:     variantSize,
			MimeType:     "video/mp4", // [UPDATED] was "video/x-matroska"
			Hash:         makeShortHash(fileName, variantSize, sentMsg.MessageID),
			UploaderID:   notifyUserID,
			LangGroupID:  &langGroupID,
			AudioLang:    label,
			Description:  fmt.Sprintf("%s — %s audio", nameNoExt, label), // [NEW] description, saved alongside title/file_id per requirement
		}
		if err := a.db.saveFile(ctx, variantRec); err != nil {
			a.logger.Warn("processAudioLanguages: save variant failed", zap.Error(err))
			lastErr = err
			continue
		}
	...
	if len(createdLabels) == 0 {
		// Nothing actually got created — do NOT tag the original file, so
		// a future /processlang retry isn't blocked by a false "already
		// ready" state. Surface the real reason so it's fixable.
		if notifyUserID != 0 {
			...
			a.pool.send(chatID, fmt.Sprintf(
				"❌ `%s` — %d audio track mile lekin koi bhi language variant successfully nahi ban paya.\n\nReason: %s\n\nSambhavit fix: server mein ffmpeg/ffprobe sahi se installed hai check karo (Dockerfile mein ffmpeg apk add hona chahiye).",
				slug, len(tracks), reason,
			))
		}
		return
	}

	// Tag the original (raw, still-MKV) file into the same group, only now
	// that at least one sibling variant actually exists — this is what makes
	// the language dropdown appear even when a visitor opens the originally
	// shared link (not one of the new hindi/english.mp4 links) directly.
	// [UPDATED] Label changed from the ISO-tag-derived track-0 language
	// (e.g. "Hindi") to a fixed "Original" — otherwise this entry would show
	// the same "Hindi" label as the new hindi.mp4 variant above, which is
	// confusing since the raw MKV isn't a reliable Hindi-only playable file.
	if _, err := a.db.setFileLangGroup(ctx, slug, langGroupID, "Original"); err != nil {
		a.logger.Warn("processAudioLanguages: tag original failed", zap.Error(err))
	}
	a.cache.delFile(ctx, slug)

	if notifyUserID != 0 {
		a.pool.send(chatID, fmt.Sprintf(
			"🌐 Language switcher ready for `%s` — %s + %d language(s) ready (%s). Watch page pe ab audio language change kar sakte ho.",
			slug, langLabel(tracks[0].Language, 0), len(createdLabels), strings.Join(createdLabels, ", "),
		))
	}
}
...
func (a *App) handleStream(w http.ResponseWriter, r *http.Request, slug string, download bool) {
	...
	if cached != nil {
		...
		a.cache.setFile(ctx, slug, &cachedFile{
			...
			LangGroupID: rec.LangGroupID, AudioLang: rec.AudioLang,
		})
	}
