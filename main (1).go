...
func (a *App) processAudioLanguages(chatID int64, notifyUserID int64, slug string, messageID int, channelID int64, fileName string, fileSize int64) {
	...
	// Tag the original file as the first/default track's language so it's
	// grouped together with the variants we're about to create.
	...
	if notifyUserID != 0 {
		a.pool.send(chatID, fmt.Sprintf("🌐 Language switcher ready for `%s` — Watch page pe ab audio language change kar sakte ho.", slug))
	}
}
