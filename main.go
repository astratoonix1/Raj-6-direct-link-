...
type Config struct {
	...
	// StringSession — BILKUL OPTIONAL. Khaali chhodo to kuch nahi hota,
	// bot token se hi (jaisa ab tak chal raha hai) sab kaam chalta rahega.
	// Jab bhi Koyeb env var mein STRING_SESSION daaloge (base64 encoded
	// gotd session), ek extra userbot MTProto worker pool mein jud jaayega
	// — isse flood-wait bachta hai kyunki real user account, bot account
	// se zyada relaxed limits pe chalta hai. NOTE: is user account ko bhi
	// DB_CHANNEL_ID wale channel mein manually add karna hoga, warna woh
	// files fetch nahi kar paayega.
	StringSession string // STRING_SESSION (optional)
	APIID         int
	APIHash       string
	AdminID       int64
	DBURI         string
	...
	// SplashAboutText — the disclaimer/about paragraph shown on the 10s
	// opening splash every /watch link shows once (password-protected or
	// not), so a first-time visitor immediately sees "this is an
	// education platform, not pirated streaming" before anything else.
	// Empty = falls back to a sensible built-in default.
	SplashAboutText string // SPLASH_ABOUT_TEXT
}
...
func loadConfig() (*Config, error) {
	...
	// STRING_SESSION — bilkul optional, isliye missing hone par koi error
	// nahi (errs mein add nahi karte). Jab set karoge tab hi use hoga.
	...
	if cfg.SplashAboutText == "" {
		cfg.SplashAboutText = "Empowering Developers Through Authentic Learning  •  " +
			...
			"specifically for aspiring developers and tech enthusiasts.  •  Our Commitment to Quality — " +
			"We believe in the power of original knowledge. To ensure you receive only the most reliable and " +
			"accurate instruction, this platform strictly prohibits the use of third-party content. Every " +
			"tutorial, code snippet, and resource you find here is carefully curated and crafted to maintain " +
			"the highest standards of integrity and clarity."
	}
...
type FileRecord struct {
	...
	Subject      string     // e.g. "Physics" — set via /tag, empty until tagged
	Chapter      string     // e.g. "Chapter 3 - Motion" — set via /tag, optional
	Year         int        // e.g. 1994 — set via /setyear, 0 until tagged
	EpisodeLabel string     // e.g. "Season 2 Episode 5" or "Part 3" — set via /setepisode, optional
}
...
// VisitorProfile is a voluntary, self-reported profile a visitor can fill
// in from the ⚙️ icon on the watch page — one per device_id, shared across
// every file (unlike ApprovalRecord, which is per file). Visible only to
// the admin, never to other visitors.
...
func (db *DB) migrate(ctx context.Context) error {
	...
	for _, q := range []string{
		...
		// visitor_profiles — one row per device_id, GLOBAL across every file
		// (unlike approvals, which is per file). A visitor fills this in
		// voluntarily from the ⚙️ profile icon on the watch page: name,
		// about, and optional contact details. Purely self-reported, shown
		// to the admin only (via /profile <access_id> or the admin-only
		// lookup icon) — never shown to other visitors.
	...
	alterQueries := []string{
		...
		`DO $$ BEGIN
			...
			END IF;
		END $$`,
		// subject/chapter — set via the /tag bot command, power the compact
		// "browse by subject" row on the watch page. Empty until tagged, so
		// existing untagged files just don't show up under any subject
		// (still fully findable via the normal search bar).
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT 1 FROM information_schema.columns
				WHERE table_name='files' AND column_name='subject') THEN
				ALTER TABLE files ADD COLUMN subject TEXT NOT NULL DEFAULT '';
			END IF;
		END $$`,
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT 1 FROM information_schema.columns
				WHERE table_name='files' AND column_name='chapter') THEN
				ALTER TABLE files ADD COLUMN chapter TEXT NOT NULL DEFAULT '';
			END IF;
		END $$`,
		`CREATE INDEX IF NOT EXISTS idx_files_subject ON files(subject) WHERE subject <> ''`,
		// year/episode_label — set via the /setyear and /setepisode bot
		// commands. year=0 and episode_label='' mean "untagged" (same
		// convention as subject/chapter above): existing files are
		// unaffected and simply don't show up under any year filter until
		// tagged. year is a plain INT (not a date) since it's just "which
		// year is this content from/about", not a timestamp.
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT 1 FROM information_schema.columns
				WHERE table_name='files' AND column_name='year') THEN
				ALTER TABLE files ADD COLUMN year INT NOT NULL DEFAULT 0;
			END IF;
		END $$`,
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT 1 FROM information_schema.columns
				WHERE table_name='files' AND column_name='episode_label') THEN
				ALTER TABLE files ADD COLUMN episode_label TEXT NOT NULL DEFAULT '';
			END IF;
		END $$`,
		`CREATE INDEX IF NOT EXISTS idx_files_year ON files(year) WHERE year <> 0`,
		// last_notified_at — powers the "📨 Notify Admin" button on the
		// pending-approval page: server-side cooldown so a visitor mashing
		// that button can't spam the admin's Telegram.
		`DO $$ BEGIN
			IF NOT EXISTS (SELECT 1 FROM information_schema.columns
				WHERE table_name='approvals' AND column_name='last_notified_at') THEN
				ALTER TABLE approvals ADD COLUMN last_notified_at TIMESTAMPTZ NULL;
			END IF;
		END $$`,
	}
...
func (db *DB) getFileByID(ctx context.Context, id string) (*FileRecord, error) {
	...
	err := db.pool.QueryRow(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count,password_hash,password_plain,subject,chapter,year,episode_label
		...
		&f.Subject, &f.Chapter, &f.Year, &f.EpisodeLabel,
	)
	return f, err
}
...
func (db *DB) searchFiles(ctx context.Context, query string, limit int) ([]*FileRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.ID, &f.MessageID, &f.ChannelID, &f.FileName,
			...
			&f.CreatedAt, &f.ExpiresAt, &f.ViewCount, &f.PasswordHash, &f.PasswordPlain,
			&f.Subject, &f.Chapter, &f.Year, &f.EpisodeLabel); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
...
// newestFiles returns the most recently uploaded files, newest first —
// powers the "New Releases" row on the watch page. Same query shape as
// topFilesByViews, just ordered by upload time instead of view count.
func (db *DB) newestFiles(ctx context.Context, limit int) ([]*FileRecord, error) {
	rows, err := db.pool.Query(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count,password_hash,password_plain,subject,chapter,year,episode_label
		FROM files ORDER BY created_at DESC LIMIT $1`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// setFileTag assigns a subject (e.g. "Physics") and optional chapter label
// (e.g. "Chapter 3 - Motion") to a file — set via the /tag bot command.
// Passing empty strings for both clears the tag (used by /untag).
// Returns false (no error) if the file id doesn't exist.
func (db *DB) setFileTag(ctx context.Context, fileID, subject, chapter string) (bool, error) {
	tag, err := db.pool.Exec(ctx, `UPDATE files SET subject=$1, chapter=$2 WHERE id=$3`, subject, chapter, fileID)
	if err != nil {
		return false, err
	}
...
// listSubjects returns every distinct subject that has at least one tagged
// file, alphabetically — powers the compact subject-icon row on the watch
// page. Untagged files (subject='') never show up here.
func (db *DB) listSubjects(ctx context.Context) ([]string, error) {
	rows, err := db.pool.Query(ctx, `SELECT DISTINCT subject FROM files WHERE subject <> '' ORDER BY subject ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// listFilesBySubject returns every file tagged with the given subject,
// grouped for display by ordering on chapter first (then upload order
// within a chapter as a tiebreak) — tip for Raj: naming chapters
// "Chapter 01", "Chapter 02", ... (zero-padded) keeps them in the right
// order here, since this is a plain text sort, not a numeric one.
func (db *DB) listFilesBySubject(ctx context.Context, subject string) ([]*FileRecord, error) {
	rows, err := db.pool.Query(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count,password_hash,password_plain,subject,chapter,year,episode_label
		FROM files WHERE subject=$1 ORDER BY chapter ASC, created_at ASC`, subject)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// setFileYear sets (year>0) or clears (year=0) the year on a file — set via
// the /setyear bot command. This only touches the year column, so it never
// clobbers an already-set episode_label (unlike a combined update would).
func (db *DB) setFileYear(ctx context.Context, fileID string, year int) (bool, error) {
	tag, err := db.pool.Exec(ctx, `UPDATE files SET year=$1 WHERE id=$2`, year, fileID)
	if err != nil {
		return false, err
	}
	return tag.RowsAffected() > 0, nil
...
// setFileEpisode sets (label != "") or clears (label == "") the episode/part
// label on a file — set via the /setepisode bot command. Only touches
// episode_label, so it never clobbers an already-set year.
func (db *DB) setFileEpisode(ctx context.Context, fileID, label string) (bool, error) {
	tag, err := db.pool.Exec(ctx, `UPDATE files SET episode_label=$1 WHERE id=$2`, label, fileID)
	if err != nil {
		return false, err
	}
	return tag.RowsAffected() > 0, nil
...
// listYears returns every distinct year that has at least one tagged file,
// most recent first — powers the Year dropdown filter on the watch page.
// Untagged files (year=0) never show up here.
func (db *DB) listYears(ctx context.Context) ([]int, error) {
	rows, err := db.pool.Query(ctx, `SELECT DISTINCT year FROM files WHERE year <> 0 ORDER BY year DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// listFilesByYear returns every file tagged with the given year, grouped
// for display by ordering on episode_label first (then upload order within
// an episode as a tiebreak) — same convention as listFilesBySubject.
func (db *DB) listFilesByYear(ctx context.Context, year int) ([]*FileRecord, error) {
	rows, err := db.pool.Query(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count,password_hash,password_plain,subject,chapter,year,episode_label
		FROM files WHERE year=$1 ORDER BY episode_label ASC, created_at ASC`, year)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) getOrCreateApproval(ctx context.Context, deviceID, slug string) (rec *ApprovalRecord, isNew bool, err error) {
	rec = &ApprovalRecord{}
	err = db.pool.QueryRow(ctx,
		`SELECT access_id, device_id, slug, visitor_name, approved, blocked, created_at, approved_at, last_notified_at
		 FROM approvals WHERE device_id=$1`, deviceID).
		Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.VisitorName, &rec.Approved, &rec.Blocked, &rec.CreatedAt, &rec.ApprovedAt, &rec.LastNotifiedAt)
	if err == nil {
...
// touchNotifyCooldown is an atomic check-and-set for the "📨 Notify Admin"
// button: it only stamps last_notified_at (and reports allowed=true) if the
// previous notify for this device was NULL or older than cooldown — so a
// visitor mashing the button can't spam the admin's Telegram no matter what
// the client-side JS does.
func (db *DB) touchNotifyCooldown(ctx context.Context, deviceID string, cooldown time.Duration) (allowed bool, err error) {
	var accessID int
	err = db.pool.QueryRow(ctx, `
		UPDATE approvals SET last_notified_at=NOW()
		WHERE device_id=$1 AND (last_notified_at IS NULL OR last_notified_at < NOW() - $2::interval)
		RETURNING access_id`,
		deviceID, fmt.Sprintf("%d seconds", int64(cooldown.Seconds())),
	).Scan(&accessID)
	if err != nil {
...
// getVisitorProfile returns this device's self-reported profile, or a
// zero-value (empty strings) one if they've never filled it in — never
// an error just for "doesn't exist yet".
func (db *DB) getVisitorProfile(ctx context.Context, deviceID string) (*VisitorProfile, error) {
	p := &VisitorProfile{DeviceID: deviceID}
	err := db.pool.QueryRow(ctx, `
		SELECT name, about, email, phone, instagram, facebook, updated_at
		FROM visitor_profiles WHERE device_id=$1`, deviceID).
		Scan(&p.Name, &p.About, &p.Email, &p.Phone, &p.Instagram, &p.Facebook, &p.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return p, nil
		}
...
// upsertVisitorProfile saves/updates a visitor's self-reported profile —
// called every time they hit Save in the ⚙️ profile panel.
func (db *DB) upsertVisitorProfile(ctx context.Context, p *VisitorProfile) error {
	_, err := db.pool.Exec(ctx, `
		INSERT INTO visitor_profiles (device_id, name, about, email, phone, instagram, facebook, updated_at)
		VALUES ($1,$2,$3,$4,$5,$6,$7,NOW())
		ON CONFLICT (device_id) DO UPDATE SET
			name=EXCLUDED.name, about=EXCLUDED.about, email=EXCLUDED.email,
			phone=EXCLUDED.phone, instagram=EXCLUDED.instagram, facebook=EXCLUDED.facebook,
			updated_at=NOW()`,
		p.DeviceID, p.Name, p.About, p.Email, p.Phone, p.Instagram, p.Facebook)
	return err
}

// getVisitorProfileByAccessID is the admin-only lookup path: given a
// 5-digit Access ID (from /user, a notification, or typed into the
// admin-only lookup icon), find that visitor's device and profile —
// joins through approvals since the profile itself is keyed by device_id,
// not access_id.
func (db *DB) getVisitorProfileByAccessID(ctx context.Context, accessID int) (*ApprovalRecord, *VisitorProfile, error) {
	rec := &ApprovalRecord{}
	err := db.pool.QueryRow(ctx, `
		SELECT access_id, device_id, slug, visitor_name, approved, blocked, created_at, approved_at, last_notified_at
		FROM approvals WHERE access_id=$1`, accessID).
		Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.VisitorName, &rec.Approved, &rec.Blocked, &rec.CreatedAt, &rec.ApprovedAt, &rec.LastNotifiedAt)
	if err != nil {
		return nil, nil, err
	}
...
func (db *DB) listApprovals(ctx context.Context, limit int) ([]*ApprovalRecord, error) {
	...
	for rows.Next() {
		rec := &ApprovalRecord{}
		if err := rows.Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.VisitorName, &rec.Approved, &rec.Blocked, &rec.CreatedAt, &rec.ApprovedAt, &rec.LastNotifiedAt); err != nil {
			return nil, err
		}
		out = append(out, rec)
	}
...
// deleteByAccessID completely wipes a visitor's record for the given
// 5-digit Access ID — used by /reject. Unlike /block (which just flags the
// row so the visitor sees "blocked"), this deletes the approval row AND
// their self-reported profile outright. If that device comes back, the
// server has no memory of it: fresh password entry, a brand-new Access ID,
// pending-approval again from scratch, and an empty profile to refill.
func (db *DB) deleteByAccessID(ctx context.Context, accessID int) (bool, error) {
	var deviceID string
	err := db.pool.QueryRow(ctx,
		`DELETE FROM approvals WHERE access_id=$1 RETURNING device_id`, accessID).
		Scan(&deviceID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return false, nil
	...
	// Best-effort — even if this fails, the approval row is already gone,
	// which is what actually forces the visitor to start over.
	_, _ = db.pool.Exec(ctx, `DELETE FROM visitor_profiles WHERE device_id=$1`, deviceID)
	return true, nil
}
...
type mtBot struct {
	...
	// isUser=true matlab yeh bot token wala worker nahi hai — STRING_SESSION
	// se aaya real user account hai. Aisi condition mein token khaali
	// rehta hai aur uske bajaye sessionData use hota hai login ke liye.
	isUser      bool
	sessionData []byte // sirf isUser=true ke liye — decoded STRING_SESSION

	ready bool
	mu    sync.Mutex
}

// stringSessionStorage — chhota sa apna session.Storage implementation.
// STRING_SESSION (base64) decode karke iske andar preload kar dete hain,
// gotd library isko load karke seedhe authorized session use kar leti
// hai — dobara phone number/OTP se login karne ki zaroorat nahi padti.
// Yeh sirf in-memory hai, kahin disk pe kuch save nahi hota.
...
func newMTProtoPool(apiID int, apiHash string, tokens []string, stringSession string, logger *zap.Logger) *MTProtoPool {
	...
	// STRING_SESSION optional hai. Khaali ho to yeh poora block skip ho
	// jaata hai aur upar wale bot-token workers jaise ke waise chalte
	// rahenge — koi issue nahi aata. Jab bhi value set karoge, ek extra
	// userbot worker pool mein jud jaayega.
	if stringSession != "" {
		data, err := base64.StdEncoding.DecodeString(stringSession)
		if err != nil {
			logger.Warn("STRING_SESSION invalid base64 hai — skip kar rahe hain, sirf bot token se chalega", zap.Error(err))
		} else {
...
func (p *MTProtoPool) startBot(ctx context.Context, apiID int, apiHash string, b *mtBot) {
	for {
		...
		if b.isUser {
			// STRING_SESSION wala worker — apna preloaded session storage use hoga
		...
		err := client.Run(ctx, func(ctx context.Context) error {
			if b.isUser {
				// Bot() call nahi karna — session pehle se authorized hai.
				// Bas confirm kar lo ki STRING_SESSION valid/expired to nahi.
				status, err := client.Auth().Status(ctx)
				if err != nil {
					return fmt.Errorf("string session status check failed: %w", err)
				}
				if !status.Authorized {
					return fmt.Errorf("STRING_SESSION expired ya invalid hai — login nahi hai")
				}
			} else {
			...
			if b.isUser {
				p.logger.Info("MTProto userbot (STRING_SESSION) authenticated")
			} else {
		...
		if err != nil && ctx.Err() == nil {
			if b.isUser {
				// STRING_SESSION kharab/expired ho to baar-baar retry karne
				// ka fayda nahi — is worker ko chhod do, baaki bot-token
				// workers bina kisi rukawat ke chalte rahenge.
				p.logger.Warn("STRING_SESSION worker band kar rahe hain (invalid/expired) — bot token se normal chalega", zap.Error(err))
				return
			}
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "tag":
		...
		if len(parts) < 2 || strings.TrimSpace(parts[1]) == "" {
			a.pool.send(msg.Chat.ID,
				"Usage: /tag <file_id> <Subject>\nYa: /tag <file_id> <Subject> / <Chapter>\n\nExample:\n/tag abc123 Physics\n/tag abc123 Physics / Chapter 3 - Motion\n\nWatch page par 'Subjects' row mein wahi subjects dikhte hain jinka kam se kam ek file tag ho.")
			return
		}
		fileID := parts[0]
	...
	case "setyear":
		...
		if len(parts) < 2 || strings.TrimSpace(parts[1]) == "" {
			a.pool.send(msg.Chat.ID,
				"Usage: /setyear <file_id> <year>\n\nExample:\n/setyear abc123 1994\n/setyear abc123 0   (clears the year)\n\nYear range: 1930-2030 — powers the Year filter dropdown on the watch page.")
			return
		}
		fileID := parts[0]
	...
	case "reject":
		...
		var accessID int
		if _, err := fmt.Sscanf(strings.TrimSpace(msg.CommandArguments()), "%d", &accessID); err != nil {
			a.pool.send(msg.Chat.ID, "Usage: /reject <access_id>\n\nYeh us visitor ka POORA record delete kar dega — usse dobara se sab kuchh (password, approval, profile) naye sirre se set karke aana hoga.")
			return
		}
		...
		a.pool.sendMD(msg.Chat.ID, fmt.Sprintf(
			"🗑️ Access ID `%05d` completely delete ho gaya\\. Yeh visitor ab bilkul naye sirre se aayega \\— naya Access ID, dobara approval, aur profile bhi khaali\\.",
			accessID,
		))
	case "clearpending":
		if msg.From.ID != a.cfg.AdminID {
...
// onVisitorActionCallback handles taps on the Approve/Block/Unblock button(s)
// attached to a "new visitor" notification — one tap instead of typing
// "/approve 12345" or "/block 12345" by hand. Data format is
// "va:<action>:<accessID>", action being appr/blk/unblk.
//
// The message is edited in place: the header (name + access ID) stays,
// only a trailing "Status:" line and the button row change — so tapping
// Block then Unblock then Block again never stacks up repeated status
// lines, it just replaces the last one.
func (a *App) onVisitorActionCallback(ctx context.Context, cb *tgbotapi.CallbackQuery) {
	answer := func(text string) {
		a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, text)) //nolint:errcheck
	}
	...
	a.pool.primary().Send(tgbotapi.NewEditMessageReplyMarkup(cb.Message.Chat.ID, cb.Message.MessageID, newKB)) //nolint:errcheck
...
func (a *App) handleHeartbeat(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// Live-tracking endpoint — must NEVER be cached by the browser or any
	// CDN in front of Koyeb, otherwise repeat calls to this same URL get
	// served from cache instead of hitting the server, and the heartbeat
	// silently stops updating Redis.
...
func (a *App) handleLiveCount(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// Same reasoning as handleHeartbeat: this response must always be fresh,
	// never cached, or the "watching now" number freezes at whatever value
	// the browser first cached instead of updating every 10s.
	w.Header().Set("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0")
	w.Header().Set("Pragma", "no-cache")
	w.Header().Set("Content-Type", "application/json")
...
// branding + a scrolling education disclaimer — before ANY /watch link
// shows its real content, password-protected or not. Reason it exists: a
// bare link with zero context (no password gate, straight to a video) was
// getting misread as pirated streaming by whoever opened it. This gives
// every first-time visitor — including someone reviewing a link — the
// same "this is an education platform, original content only" context up
// front, regardless of which link they were handed.
//
// After 10s, the page sets the "seen" cookie and reloads nextURL (the
// exact same watch URL) — this time hasSplashCookie is true, so
// handleWatch just falls straight through to its normal password/approval/
// video logic, completely unchanged. Already-returning visitors (cookie
// already set from an earlier visit) never see this at all.
...
// astratoonixSubjectTiles renders the faint tiled background of subject/
// topic names used on both the password page and the pending-approval
// page — signals "this whole site is an education platform" at a glance,
// shared so both pages always show the exact same list.
...
// astratoonixGateCSS is the CSS shared by both gate pages (password prompt
// and pending-approval) — same fonts, same dark theme, same boot splash,
// same subject-tile background, same card/badge look. Kept in one place so
// the two pages can never visually drift apart; each caller appends a small
// amount of its own page-specific CSS (input fields, unlock button, etc.)
// after this.
//
// The boot splash is JS-driven (via astratoonixBootJS), not pure CSS
// animation-delay — that's deliberate. A pure-CSS delay chain (typewriter
// finishes at 1.1s, but the boot overlay's own fade-out was separately
// timed to start at 0.1s) is exactly what caused the original bug: the
// overlay disappeared mid-typewriter, cutting the brand name off before it
// finished. Driving both from one JS timer guarantees the overlay can never
// hide before the full "ASTRATOONIX" + tagline have been visible.
...
/* ---------- Boot / intro splash — full 10s, JS-timed (see astratoonixBootJS) ---------- */
#boot {
...
// astratoonixBootHTML is the boot-splash markup itself, shared by both gate
// pages — "ASTRATOONIX", an educational-platform tag, and the "BUILT BY
// RAJ" credit, sitting on top of the tiled subject-name background.
...
// astratoonixBootJS drives the boot splash timing with one JS timer instead
// of chained CSS animation-delays (see astratoonixGateCSS for why). When
// playBoot is true: the full "ASTRATOONIX" typewriter + tagline play out
// and stay fully visible for a solid 10 seconds total before the card
// fades in — so nobody can miss it, even on a quick glance. When false
// (used for a wrong-password retry, so a mistyped password doesn't force
// the visitor to sit through the intro again just to see the error), the
// splash is skipped entirely and the card shows immediately.
//
// Also explicitly (re)starts the optional instructional video (#ppv, if
// PASSWORD_PROMPT_VIDEO_URL is set) the moment the card is revealed.
// Browsers can pause — or simply never start — an autoplay video that's
// sitting behind the boot overlay for a full 10 seconds; relying on the
// `autoplay` attribute alone was silently failing once the splash got
// this much longer. Calling .play() here guarantees it's actually
// running at the moment it becomes visible, regardless of what the
// browser did with it while it was covered.
...
%s
	...
	// visitor even reads a word. Shared helper so this always matches the
	// pending-approval page exactly.
...
// admin hasn't run /approve <accessID> for this device yet. Polls
// /approval-status/<slug> every 6s in the background and reloads only once
// actually approved/blocked — deliberately NOT a <meta refresh>, because a
// blind full-page reload every 6s would restart the 10s boot splash over
// and over and the visitor would never actually see the card underneath.
// Shares its whole visual identity (boot splash, subject tiles, badges,
// card style) with renderPasswordPrompt via the astratoonix* helpers, so
// a visitor never sees two different-looking "halves" of what's obviously
// meant to be one platform — and both pages carry the same "this is an
// education platform, not spam" messaging up front.
...
// Quietly polls in the background instead of a blind full-page reload —
// reloads (for real) only once actually approved or blocked.
setInterval(function() {
...
// approve — and two tappable buttons so approving/blocking never needs
// typing "/approve 12345" by hand. Existing/repeat visitors don't spam
// this — isNew is only true the first time getOrCreateApproval ever sees
// that device.
//
...
// notifyVisitorRequest sends the SAME styled notification (with the same
// Approve/Block buttons) as notifyNewAccessID, but unconditionally — it's
// fired by the visitor deliberately tapping "📨 Notify Admin" on the
// pending-approval page, not by the isNew-device check. The caller
// (handleNotify) is responsible for the per-device cooldown, so this
// function itself doesn't rate-limit.
...
// handleApprovalStatus is a tiny JSON poll endpoint the pending-approval
// page calls every 6s in the background (see renderPendingApproval) to
// find out if this device has been approved or blocked yet, WITHOUT doing
// a full page reload — a blind reload would also restart the 10s boot
// splash every single time, so the visitor would never get past it.
...
// handleNotify backs the "📨 Notify Admin" button on the pending-approval
// page — POST /notify/<slug>. Saves the name (if changed) and pings the
// admin, subject to a per-device cooldown (touchNotifyCooldown) so mashing
// the button can't spam Telegram. Always responds with JSON, never HTML —
// this is called via fetch(), not a form navigation.
func (a *App) handleNotify(w http.ResponseWriter, r *http.Request, slug string) {
	...
	if approval.Blocked || approval.Approved {
		// Already resolved either way — nothing to notify about.
		fmt.Fprint(w, `{"ok":true}`)
		return
	}
	if visitorName != approval.VisitorName {
		a.db.setApprovalName(ctx, deviceID, visitorName) //nolint:errcheck
	}
...
// handleProfile is the self-service profile endpoint for the ⚙️ icon on
// the watch page — GET returns the current device's own profile, POST
// upserts it. Keyed entirely by the device_id cookie already on the
// request, so a visitor can only ever read/write THEIR OWN profile through
// this endpoint — there's deliberately no "id"/"device" parameter here.
// Looking up someone else's profile is a separate, admin-only endpoint
// (handleProfileLookup).
func (a *App) handleProfile(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// Keep the approval row's name in sync too, so /user, the pending
		// page, and Telegram notifications all show whatever name was last
		// saved here — one name, everywhere.
		if p.Name != "" {
			a.db.setApprovalName(ctx, deviceID, p.Name) //nolint:errcheck
		}
		fmt.Fprint(w, `{"ok":true}`)
		return
	}
...
// handleProfileLookup is the admin-only "look up any visitor's profile by
// Access ID" endpoint — GET /profile/lookup?id=<accessID>&token=<dashboard
// token>. Gated by the SAME token as /admin: without it, this always
// returns 403, regardless of how the request is made. This is deliberate —
// students' phone numbers/emails/Instagram handles must never be
// reachable just by guessing or incrementing a 5-digit Access ID.
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// One-time opening splash — every /watch link (password-protected or
	// not) shows this exactly once per browser, before anything else.
	// After it, this exact same URL gets reloaded and falls straight
	...
	// freshly submitted password. r.FormValue reads POST body first (the
	// form now submits via POST, so the password never touches the address
	// bar / browser history / shared-link text) and falls back to the URL
	// query only for old bookmarked ?pw= links from before this fix.
	if passwordHash != nil && *passwordHash != "" {
		visitorName := strings.TrimSpace(r.FormValue("name"))
...
// handleRows powers the Netflix-style "Trending" and "New Releases" rows on
// the watch page. Returns JSON: {"trending":[...], "new_releases":[...]}.
// Each entry has the same {id, name, size, locked} shape as /search results
// so the frontend can reuse one card-render function for all three rows.
func (a *App) handleRows(w http.ResponseWriter, r *http.Request) {
	...
	trending, err := a.db.topFilesByViews(ctx, 12)
	if err != nil {
		a.logger.Error("topFilesByViews failed", zap.Error(err))
	}
	newest, err := a.db.newestFiles(ctx, 12)
	if err != nil {
...
// handleSubjects powers the compact "Subjects" row on the watch page —
// two modes on one endpoint:
//   GET /subjects              -> {"subjects": ["Physics", "Chemistry", ...]}
//   GET /subjects?name=Physics -> {"subject": "Physics", "files": [...]}
// Only subjects with at least one tagged file (via /tag) ever appear.
...
// handleYears powers the Year dropdown filter on the watch page — two modes
// on one endpoint, same convention as /subjects:
//   GET /years            -> {"years": [2026, 2020, 1994, ...]}  (desc)
//   GET /years?year=1994  -> {"year": 1994, "files": [...]}
// Only years with at least one tagged file (via /setyear) ever appear.
...
// handleAdmin renders the admin dashboard: stats, a pending-visitors list
// with one-tap Approve/Block/Unblock, and a per-file table with one-tap
// Delete — so the routine stuff doesn't need the Telegram bot open. Gated
// by a token so it isn't publicly browsable — get the link via /dashboard.
//
// Actions are plain GET links (?approve=/?block=/?unblock=/?delfile=) since
// the whole page is already token-gated to just the admin; each one
// redirects back to the clean dashboard URL afterward so the page's own
// 10s auto-refresh (or hitting back/forward) never re-fires the same
// action twice.
func (a *App) handleAdmin(w http.ResponseWriter, r *http.Request) {
	...
	if idStr := r.URL.Query().Get("approve"); idStr != "" {
		if id, err := strconv.Atoi(idStr); err == nil {
			a.db.approveByID(ctx, id) //nolint:errcheck
		}
		redirectBack()
		return
	}
	if idStr := r.URL.Query().Get("block"); idStr != "" {
		if id, err := strconv.Atoi(idStr); err == nil {
			a.db.blockByID(ctx, id, true) //nolint:errcheck
		}
		redirectBack()
		return
	}
	if idStr := r.URL.Query().Get("unblock"); idStr != "" {
		if id, err := strconv.Atoi(idStr); err == nil {
			a.db.blockByID(ctx, id, false) //nolint:errcheck
		}
		redirectBack()
		return
	}
	if fid := r.URL.Query().Get("delfile"); fid != "" {
		if rec, err := a.db.getFileByID(ctx, fid); err == nil {
			a.db.deleteFileByMsgID(ctx, rec.MessageID) //nolint:errcheck
...
// deviceCookieName / passwordCookieName / splashCookieName — cookies used
// for unique-device view counting, remembering a correctly-entered link
// password, and remembering the opening splash has already been shown.
...
// hasSplashCookie / setSplashCookie gate the 10s opening splash (ASTRATOONIX
// branding + education disclaimer) — shown once per browser, the very
// first time it ever opens ANY /watch link on the site, password-protected
// or not. Deliberately a separate cookie from the password/approval ones:
// it's about first impressions site-wide, not about any one file or gate.
...
func setSplashCookie(w http.ResponseWriter) {
	http.SetCookie(w, &http.Cookie{
		...
		Value:    "1",
		Path:     "/",
		MaxAge:   10 * 365 * 24 * 60 * 60, // ~10 years — same "seen once, remembered" idea as the device cookie
		HttpOnly: false,
		SameSite: http.SameSiteLaxMode,
	})
}
...
func main() {
	...
	// Start MTProto pool — MAIN BOT se streaming (worker bots ka MTProto
	// nahi — unhe channel ka access nahi hoga!). Agar STRING_SESSION set
	// hai to ek extra userbot worker bhi is pool mein jud jaayega —
	// warna sab kuch bot token se hi pehle jaisa chalta rahega.
