...
// streams only). No re-encoding happens (-c copy), so this is fast and
// lossless — just a container repack.
...
func (a *App) processAudioLanguages(chatID int64, notifyUserID int64, slug string, messageID int, channelID int64, fileName string, fileSize int64) {
	...
	for i, track := range tracks[1:] {
		trackIdx := i + 1
		label := langLabel(track.Language, trackIdx)
		outPath := filepath.Join(tmpDir, fmt.Sprintf("variant_%d%s", trackIdx, ext))
		...
		fileBytes := tgbotapi.FileReader{Name: fmt.Sprintf("%s [%s]%s", nameNoExt, label, ext), Reader: f}
		...
		variantRec := &FileRecord{
			...
			ChannelID:    toInternalChannelID(a.cfg.DBChannelID),
			FileName:     fmt.Sprintf("%s [%s]%s", nameNoExt, label, ext),
	...
	// Tag the original file as the first/default track's language, only now
	// that at least one sibling variant actually exists.
	if _, err := a.db.setFileLangGroup(ctx, slug, langGroupID, langLabel(tracks[0].Language, 0)); err != nil {
