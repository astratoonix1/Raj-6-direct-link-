...
type Config struct {
	...
	AudioLangProcessingEnabled bool
	AudioLangMaxFileSize       int64
	HLSStorageDir              string
	HLSBaseURL                 string
}
...
func loadConfig() (*Config, error) {
	...
	// Audio-language extraction: off by default because it needs ffmpeg in
	// the image and downloads+reuploads the whole file, which costs time,
	// bandwidth and disk. Enable with AUDIO_LANG_PROCESSING=1 once ffmpeg is
	// available in the deployed container.
	cfg.AudioLangProcessingEnabled = strings.TrimSpace(os.Getenv("AUDIO_LANG_PROCESSING")) == "1"
	cfg.AudioLangMaxFileSize = int64(envInt("AUDIO_LANG_MAX_FILE_MB", 6000)) * 1024 * 1024

	// HLS multi-audio-language streaming. Segments are written to local
	// (ephemeral) disk and served directly by this app — on Koyeb's free
	// tier there's no persistent volume, so this folder is wiped on every
	// restart/redeploy; re-run /processlang for a file after a redeploy if
	// its language switcher stops working.
	cfg.HLSStorageDir = strings.TrimSpace(os.Getenv("HLS_STORAGE_DIR"))
	if cfg.HLSStorageDir == "" {
		cfg.HLSStorageDir = "/hlsdata"
	}
	cfg.HLSBaseURL = strings.TrimSpace(os.Getenv("HLS_BASE_URL")) // optional override; defaults to request's own host at runtime
...
// isVideoLikeMime reports whether a mime type is worth probing for extra
// audio tracks. Skips images, plain audio files, documents, etc.
//
// Telegram's reported mime_type for .mkv uploads is unreliable — clients
// send anything from "video/x-matroska" to "application/x-matroska" to a
// blank string, so we also fall back to checking the filename extension
// rather than trusting mime_type alone.
func isVideoLikeMime(mimeType string) bool {
	mimeType = strings.ToLower(strings.TrimSpace(mimeType))
	if strings.HasPrefix(mimeType, "video/") {
		return true
	}
	switch mimeType {
	case "application/octet-stream", "application/x-matroska", "application/mp4", "application/vnd.rn-realmedia":
		return true
	}
	return false
}

// looksLikeVideoFile is a filename-extension fallback for isVideoLikeMime,
// used because Telegram's reported mime_type for video containers
// (especially .mkv) is inconsistent across clients.
func looksLikeVideoFile(fileName string) bool {
	ext := strings.ToLower(filepath.Ext(fileName))
	switch ext {
	case ".mkv", ".mp4", ".avi", ".mov", ".webm", ".m4v", ".ts", ".flv", ".wmv", ".3gp":
		return true
	}
	return false
}
...
type FileRecord struct {
	...
	LangGroupID   *string    `bson:"lang_group_id,omitempty" json:"lang_group_id,omitempty"`
	AudioLang     string     `bson:"audio_lang,omitempty" json:"audio_lang,omitempty"`
	HLSMasterPath string     `bson:"hls_master_path,omitempty" json:"hls_master_path,omitempty"`
...
func (db *DB) saveFile(ctx context.Context, f *FileRecord) error {
	...
	update := bson.M{
		"$set": bson.M{
			...
			"lang_group_id":  f.LangGroupID,
			"audio_lang":     f.AudioLang,
			"hls_master_path": f.HLSMasterPath,
...
// setFileLangGroup tags an existing file (typically the originally uploaded
// file) as belonging to a language-variant group, so it shows up alongside
// the extracted single-audio-track variants in the watch page's language
// switcher.
func (db *DB) setFileLangGroup(ctx context.Context, fileID, langGroupID, audioLang string) (bool, error) {
	res, err := db.files.UpdateOne(ctx, bson.M{"_id": fileID}, bson.M{"$set": bson.M{"lang_group_id": langGroupID, "audio_lang": audioLang}})
	if err != nil {
		return false, err
	}
	return res.MatchedCount > 0, nil
...
// setFileHLSPath tags a file with the URL path of its multi-audio HLS
// bundle (e.g. "/hls/<groupid>/master.m3u8"), once it's been built and
// written to local disk.
func (db *DB) setFileHLSPath(ctx context.Context, fileID, hlsPath string) (bool, error) {
	res, err := db.files.UpdateOne(ctx, bson.M{"_id": fileID}, bson.M{"$set": bson.M{"hls_master_path": hlsPath}})
	if err != nil {
		return false, err
	}
	return res.MatchedCount > 0, nil
...
// listLangGroupFiles returns every audio-language variant belonging to the
// same original upload (linked by LangGroupID), used to build the in-page
// language switcher on the watch page.
func (db *DB) listLangGroupFiles(ctx context.Context, langGroupID string) ([]*FileRecord, error) {
	opts := options.Find().SetSort(bson.D{{Key: "created_at", Value: 1}})
	cursor, err := db.files.Find(ctx, bson.M{"lang_group_id": langGroupID}, opts)
	if err != nil {
		return nil, err
	}
	defer cursor.Close(ctx)
...
type cachedFile struct {
	...
	LangGroupID  *string    `json:"lang_group_id,omitempty"`
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "processlang":
		...
		if !a.cfg.AudioLangProcessingEnabled {
			a.pool.send(msg.Chat.ID, "❌ Language processing abhi OFF hai. AUDIO_LANG_PROCESSING=1 set karke redeploy karo pehle.")
			return
		}
		slug := strings.TrimSpace(msg.CommandArguments())
		if slug == "" {
			a.pool.send(msg.Chat.ID, "Usage: /processlang <file_id>\n\nPurani (pehle se uploaded) file ke liye language-switcher backfill karta hai — dobara upload karne ki zaroorat nahi.")
			return
		}
		rec, err := a.db.getFileByID(ctx, slug)
		if err != nil {
		...
		if rec.HLSMasterPath != "" {
			// Verify the bundle actually still exists on disk — Koyeb's
			// free tier has no persistent volume, so a redeploy wipes it
			// even though the DB still remembers the old path.
			if _, statErr := os.Stat(filepath.Join(a.cfg.HLSStorageDir, strings.TrimPrefix(rec.HLSMasterPath, "/hls/"))); statErr == nil {
				a.pool.send(msg.Chat.ID, "ℹ️ Is file ke liye language switcher already ready hai.")
				return
			}
			a.pool.send(msg.Chat.ID, "⚠️ Pehle ka HLS bundle mil nahi raha (shayad redeploy ke baad disk clear ho gaya) — dobara bana raha hoon.")
		} else if rec.LangGroupID != nil && *rec.LangGroupID != "" {
			// Legacy per-file variants from an older version of this
			// feature — don't just trust the flag, verify siblings exist.
			members, mErr := a.db.listLangGroupFiles(ctx, *rec.LangGroupID)
			if mErr == nil && len(members) > 1 {
				a.pool.send(msg.Chat.ID, "ℹ️ Is file ke liye language switcher already ready hai (purane format mein).")
				return
			}
			a.pool.send(msg.Chat.ID, "⚠️ Pehle ka attempt adhoora tha (koi variant nahi mila tha) — dobara try kar raha hoon.")
		}
		if !isVideoLikeMime(rec.MimeType) && !looksLikeVideoFile(rec.FileName) {
			a.pool.send(msg.Chat.ID, "❌ Yeh video-type file nahi lagti, language processing skip.")
			return
		}
		if rec.FileSize <= 0 || rec.FileSize > a.cfg.AudioLangMaxFileSize {
			a.pool.send(msg.Chat.ID, fmt.Sprintf("❌ File size limit (%s) se bahar hai, skip.", formatSize(a.cfg.AudioLangMaxFileSize)))
			return
		}
		a.pool.send(msg.Chat.ID, fmt.Sprintf("⏳ `%s` ke liye language check shuru — download + ffprobe ho raha hai background mein, thoda time lagega badi files ke liye.", slug))
		go a.processAudioLanguages(msg.Chat.ID, msg.From.ID, rec.ID, rec.MessageID, rec.ChannelID, rec.FileName, rec.FileSize)
	}
}
...
// ---------------------------------------------------------------------------
// In-page audio-language switching.
//
// Browsers (especially Chrome on Android) do not support switching between
// embedded audio tracks of a video, even though external players like VLC or
// PlayIt can. To make language switching work inside our own web player, we
// detect extra audio tracks with ffprobe right after a file is stored, and
// for every extra language we remux (video copy + that one audio track,
// -c copy so it's just a fast container repack, not a re-encode) into its
// own standalone file. Each variant is uploaded to the storage channel like
// any other file and linked back via LangGroupID, so the existing streaming,
// caching, and approval code needs zero changes — the watch page just gets a
// language dropdown that swaps the player's src between these sibling URLs,
// exactly like the existing quality switcher already does.
// ---------------------------------------------------------------------------

type audioTrackInfo struct {
	StreamIndex int    // ffmpeg -map 0:a:<StreamIndex> (0-based, audio streams only)
	Language    string // ISO 639 code from container metadata, may be empty
}

var isoLangNames = map[string]string{
	"hin": "Hindi", "eng": "English", "ben": "Bengali", "bn": "Bengali",
	"tam": "Tamil", "tel": "Telugu", "mar": "Marathi", "guj": "Gujarati",
	"pan": "Punjabi", "urd": "Urdu", "asm": "Assamese", "kan": "Kannada",
	"mal": "Malayalam", "ori": "Odia", "nld": "Dutch", "dut": "Dutch",
	"fre": "French", "fra": "French", "spa": "Spanish", "ger": "German",
	"deu": "German", "jpn": "Japanese", "kor": "Korean", "chi": "Chinese",
	"zho": "Chinese", "ara": "Arabic", "rus": "Russian", "por": "Portuguese",
}

func langLabel(code string, fallbackIndex int) string {
	code = strings.ToLower(strings.TrimSpace(code))
	if name, ok := isoLangNames[code]; ok {
		return name
	}
	if code != "" && code != "und" {
		return strings.ToUpper(code)
	}
	return fmt.Sprintf("Track %d", fallbackIndex+1)
}

// probeAudioTracksFromReader runs ffprobe over a streamed reader (no local
// file needed) and returns the audio streams found. We only ever feed this
// a small header chunk (see probeHeaderBytes in processAudioLanguages), not
// the whole file — MKV/MP4 track metadata lives near the start of the
// container, so a partial read is enough to enumerate audio tracks without
// downloading gigabytes just to check languages.
func probeAudioTracksFromReader(ctx context.Context, r io.Reader) ([]audioTrackInfo, error) {
	...
	for i, s := range parsed.Streams {
		tracks = append(tracks, audioTrackInfo{StreamIndex: i, Language: s.Tags.Language})
	}
	return tracks, nil
}

// openTelegramRange opens a streamed reader for a byte range of a stored
// Telegram file, without ever touching local disk. Reused for both the
// lightweight header probe and the single full pass used to demux every
// audio-language variant at once.
func (a *App) openTelegramRange(ctx context.Context, channelID int64, messageID int, fileSize, start, end int64) (*TgFileReader, error) {
	location, tgSize, err := a.mtPool.getFileLocation(ctx, channelID, messageID)
	if err != nil {
		return nil, fmt.Errorf("getFileLocation: %w", err)
	}
	...
	api := bot.api
	bot.mu.Unlock()
	return newTgFileReader(ctx, api, a.cfg, location, fileSize, start, end), nil
}

// buildHLSBundle streams the full source file from Telegram exactly ONCE
// into ffmpeg, which produces a single multi-audio-track HLS bundle — one
// video-only variant plus one audio-only variant per language, all sharing
// the same 6-second segment boundaries — and writes the segments directly
// to local disk under <HLSStorageDir>/<groupID>/. No temp copy of the full
// source file is ever created; only the (much smaller, since -c:v copy
// doesn't re-encode video) HLS output sits on disk, and only for as long as
// this directory exists.
func (a *App) buildHLSBundle(ctx context.Context, channelID int64, messageID int, fileSize int64, tracks []audioTrackInfo, groupID string) (string, error) {
	outDir := filepath.Join(a.cfg.HLSStorageDir, groupID)
	if err := os.MkdirAll(outDir, 0755); err != nil {
		return "", fmt.Errorf("mkdir outDir: %w", err)
	}
	// ffmpeg's HLS muxer needs each variant's subdirectory to exist before
	// it can write segment files into it. Variant indices follow the order
	// given to -var_stream_map: audio tracks 0..N-1, then video at index N.
	for i := 0; i <= len(tracks); i++ {
		if err := os.MkdirAll(filepath.Join(outDir, fmt.Sprintf("stream_%d", i)), 0755); err != nil {
			os.RemoveAll(outDir)
			return "", fmt.Errorf("mkdir stream dir: %w", err)
		}
	}

	srcReader, err := a.openTelegramRange(ctx, channelID, messageID, fileSize, 0, fileSize-1)
	if err != nil {
		os.RemoveAll(outDir)
	...
	args := []string{"-i", "pipe:0", "-map", "0:v:0"}
	for i := range tracks {
		args = append(args, "-map", fmt.Sprintf("0:a:%d", i))
	}
	args = append(args, "-c:v", "copy", "-c:a", "aac", "-b:a", "128k")

	streamMapParts := make([]string, 0, len(tracks)+1)
	for i, tr := range tracks {
		label := langLabel(tr.Language, i)
		langCode := strings.ToLower(strings.TrimSpace(tr.Language))
		if langCode == "" || langCode == "und" {
			langCode = fmt.Sprintf("trk%d", i+1)
		}
		part := fmt.Sprintf("a:%d,agroup:audio,name:%s,language:%s", i, label, langCode)
		if i == 0 {
			part += ",default:yes"
		}
		streamMapParts = append(streamMapParts, part)
	}
	...
	args = append(args,
		...
		"-hls_segment_filename", filepath.Join(outDir, "stream_%v", "segment_%03d.ts"),
	...
	if err := cmd.Run(); err != nil {
		...
		if len(reason) > 500 {
			reason = reason[len(reason)-500:] // ffmpeg's actual error is usually at the end
		}
		return "", fmt.Errorf("ffmpeg: %w: %s", err, reason)
	}

	masterPath := filepath.Join(outDir, "master.m3u8")
	if _, err := os.Stat(masterPath); err != nil {
		os.RemoveAll(outDir)
		return "", fmt.Errorf("master playlist missing after ffmpeg run")
	}

	return fmt.Sprintf("/hls/%s/master.m3u8", groupID), nil
}

// processAudioLanguages runs in the background after a file is stored. If
// the file has more than one embedded audio track, it builds a single HLS
// (chunked) bundle covering every language — the source is read from
// Telegram exactly once, piped straight into ffmpeg, which writes segments
// directly to local disk. This matters because small hosts (e.g. Koyeb's
// free/eco tier) don't have the time/disk budget to download a multi-GB
// file wholesale — this design never buffers or re-downloads the full
// source more than once, regardless of how many languages are involved.
func (a *App) processAudioLanguages(chatID int64, notifyUserID int64, slug string, messageID int, channelID int64, fileName string, fileSize int64) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Minute)
	defer cancel()

	notify := func(format string, args ...any) {
		if notifyUserID != 0 {
			a.pool.send(chatID, fmt.Sprintf(format, args...))
		}
	}

	// Step 1: probe. Only fetch a small header chunk from Telegram — MKV/MP4
	// track metadata sits near the start of the file, so we don't need to
	// download gigabytes just to find out how many audio languages exist.
	const probeHeaderBytes = 64 * 1024 * 1024 // 64MB
	probeEnd := fileSize - 1
	if fileSize > probeHeaderBytes {
		probeEnd = probeHeaderBytes - 1
	}
	probeReader, err := a.openTelegramRange(ctx, channelID, messageID, fileSize, 0, probeEnd)
	if err != nil {
		a.logger.Warn("processAudioLanguages: probe range open failed", zap.Error(err), zap.String("slug", slug))
		notify("❌ %s — Telegram se file ka header fetch nahi ho paya, dobara try karo.", slug)
		return
	}
	tracks, err := probeAudioTracksFromReader(ctx, probeReader)
	probeReader.Close()
	if err != nil {
		a.logger.Warn("processAudioLanguages: probe failed", zap.Error(err), zap.String("slug", slug))
		notify("❌ %s — ffprobe fail ho gaya, ffmpeg image mein sahi se installed hai check karo.\n\nReason: %v", slug, err)
		return
	}
	if len(tracks) < 2 {
		notify("ℹ️ %s mein sirf ek hi audio track mila — language switcher ki zaroorat nahi.", slug)
		return
	}

	// Step 2: build a single HLS bundle (one video-only variant + one
	// audio-only variant per language, all sharing the same 6-second
	// segments) in ONE streamed pass over the source — read once from
	// Telegram, piped straight into ffmpeg, which writes segments directly
	// to local disk. Switching languages in the player then means hls.js
	// swapping which audio segments it fetches — instant, no reload, no
	// re-download, and no giant file ever sits in memory or gets
	// downloaded more than once.
	groupID := uuid.New().String()
	hlsPath, err := a.buildHLSBundle(ctx, channelID, messageID, fileSize, tracks, groupID)
	if err != nil {
		a.logger.Warn("processAudioLanguages: HLS build failed", zap.Error(err), zap.String("slug", slug))
		notify("❌ %s — HLS build fail ho gaya.\n\nReason: %v\n\nSambhavit fix: server mein ffmpeg sahi se installed hai check karo (Dockerfile mein ffmpeg apk add hona chahiye).", slug, err)
		return
	}

	if _, err := a.db.setFileHLSPath(ctx, slug, hlsPath); err != nil {
		a.logger.Warn("processAudioLanguages: tag HLS path failed", zap.Error(err))
	}
	a.cache.delFile(ctx, slug)

	labels := make([]string, len(tracks))
	for i, tr := range tracks {
		labels[i] = langLabel(tr.Language, i)
	}
	notify("🌐 Language switcher ready for %s — %d language(s) available (%s). Watch page pe ab audio language change kar sakte ho, bina reload/re-download ke.",
		slug, len(tracks), strings.Join(labels, ", "))
}
...
func (a *App) onFile(ctx context.Context, msg *tgbotapi.Message) {
	...
	// Kick off audio-language detection/extraction in the background so the
	// user gets their link immediately. Only worth doing for video-ish files
	// under a sane size (large downloads+reuploads cost time and disk).
	if a.cfg.AudioLangProcessingEnabled && (isVideoLikeMime(fi.mimeType) || looksLikeVideoFile(fi.fileName)) && fi.fileSize > 0 {
		if fi.fileSize <= a.cfg.AudioLangMaxFileSize {
			go a.processAudioLanguages(msg.Chat.ID, msg.From.ID, slug, fwdMsg.MessageID, channelID, fi.fileName, fi.fileSize)
		} else {
			a.pool.send(msg.Chat.ID, fmt.Sprintf(
				"ℹ️ %s — language check skip ho gaya, file size (%s) limit (%s) se bahar hai. Baad mein /processlang %s se manually try kar sakte ho.",
				fi.fileName, formatSize(fi.fileSize), formatSize(a.cfg.AudioLangMaxFileSize), slug,
			))
		}
	}
}
...
// langOption is one entry in the watch page's audio-language switcher
// dropdown — each points at a standalone single-audio-track remuxed file.
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	if cached != nil {
		...
		langGroupID = cached.LangGroupID
		hlsMasterPath = cached.HLSMasterPath
	} else {
		rec, err := a.db.getFileByID(ctx, slug)
		if err != nil {
			http.Error(w, "File not found.", http.StatusNotFound)
		...
		passwordHash = rec.PasswordHash
		groupID = rec.GroupID
		langGroupID = rec.LangGroupID
		hlsMasterPath = rec.HLSMasterPath
		a.cache.setFile(ctx, slug, &cachedFile{
			MessageID: rec.MessageID, ChannelID: rec.ChannelID,
			FileName: rec.FileName, FileSize: rec.FileSize, MimeType: rec.MimeType,
			ExpiresAt: expiresAt, PasswordHash: passwordHash,
			GroupID: rec.GroupID, QualityLabel: rec.QualityLabel, QualityRank: rec.QualityRank,
			LangGroupID: rec.LangGroupID, AudioLang: rec.AudioLang, HLSMasterPath: rec.HLSMasterPath,
		})
	}
	...
	data.LanguagesJSON = template.JS("[]")
	if langGroupID != nil && *langGroupID != "" {
		if members, lErr := a.db.listLangGroupFiles(ctx, *langGroupID); lErr == nil && len(members) > 1 {
			opts := make([]langOption, 0, len(members))
			for _, m := range members {
				label := m.AudioLang
				if label == "" {
					label = "Default"
			...
			if b, mErr := json.Marshal(opts); mErr == nil {
				data.LanguagesJSON = template.JS(b)
			}
		}
	}

	// HLS multi-audio bundle (preferred path): single video, instant audio
	// switching via hls.js, no reload. Takes priority over the older
	// separate-file LanguagesJSON switcher above when present.
	if hlsMasterPath != "" {
		data.HLSMasterURL = base + hlsMasterPath
	}
...
// handleHLSFile serves HLS playlists and .ts segments straight off local
// disk (see buildHLSBundle). Files live under <HLSStorageDir>/<groupID>/...
// and are requested by hls.js as /hls/<groupID>/... — this just maps that
// URL path to the file on disk with the right content type and Range
// support (http.ServeContent handles Range headers for us via the
// io.ReadSeeker os.File already satisfies).
func (a *App) handleHLSFile(w http.ResponseWriter, r *http.Request) {
	rel := strings.TrimPrefix(r.URL.Path, "/hls/")
	if rel == "" || strings.Contains(rel, "..") {
		http.NotFound(w, r)
		return
	}
	fullPath := filepath.Join(a.cfg.HLSStorageDir, rel)
	...
	case strings.HasSuffix(rel, ".m3u8"):
		...
		w.Header().Set("Cache-Control", "no-cache") // playlists can be small/updated; segments below are immutable
...
func main() {
	...
	if cfg.AudioLangProcessingEnabled {
		if err := os.MkdirAll(cfg.HLSStorageDir, 0755); err != nil {
			logger.Warn("could not create HLS storage dir — language-switcher HLS builds will fail", zap.Error(err), zap.String("dir", cfg.HLSStorageDir))
		} else {
			logger.Info("✅ HLS storage dir ready", zap.String("dir", cfg.HLSStorageDir))
		}
	}
	...
	mux.HandleFunc("/hls/", app.handleHLSFile)
