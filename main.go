...
func (c *Cache) setAdminReply(ctx context.Context, deviceID, message string) {
	c.client.Set(ctx, "adminreply:"+deviceID, message, 3*24*time.Hour)
}

func (c *Cache) popAdminReply(ctx context.Context, deviceID string) (string, bool) {
	key := "adminreply:" + deviceID
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "reply":
		...
		parts := strings.SplitN(args, " ", 2)
		if len(parts) < 2 {
			a.pool.send(msg.Chat.ID, "Usage: /reply <access_id> <message>\n\nYeh message us visitor ki website pe (agli baar page open/refresh karne pe) dikh jaayega.")
			return
		}
		var accessID int
		if _, err := fmt.Sscanf(parts[0], "%d", &accessID); err != nil {
			a.pool.send(msg.Chat.ID, "Usage: /reply <access_id> <message>")
			return
		}
		replyText := strings.TrimSpace(parts[1])
		if replyText == "" {
			a.pool.send(msg.Chat.ID, "Message khaali nahi ho sakta.")
			return
		}
		approval, err := a.db.getApprovalByAccessID(ctx, accessID)
		if err != nil || approval == nil {
			a.pool.send(msg.Chat.ID, fmt.Sprintf("❌ Access ID `%05d` nahi mila.", accessID))
			return
		}
		a.cache.setAdminReply(ctx, approval.DeviceID, replyText)
		a.pool.send(msg.Chat.ID, fmt.Sprintf("✅ Reply save ho gaya — Access ID %05d wale visitor ko unki website pe dikhega.", accessID))
	case "reject":
		if msg.From.ID != a.cfg.AdminID {
			a.pool.send(msg.Chat.ID, "❌ Admin only.")
			return
...
func (a *App) handleContactCheck(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Cache-Control", "no-store")
	deviceID := getOrSetDeviceID(w, r)
	reply, ok := a.cache.popAdminReply(ctx, deviceID)
	if !ok {
		w.Write([]byte(`{"reply":null}`))
		return
	}
...
func main() {
	...
	mux.HandleFunc("/contact/check", app.handleContactCheck)
