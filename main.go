...
// watchPageHTML is compiled directly into the binary at build time from the
// index.html file sitting next to main.go. This means the template served
// to visitors can never drift from what was actually built/deployed —
// there is no runtime disk path (Docker layer, working directory, stale
// volume mount, etc.) that can silently serve an old copy instead.
//
//go:embed index.html
var watchPageHTML string

var watchPageTmpl = template.Must(template.New("watch").Parse(watchPageHTML))
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "gencode":
		...
		if convErr != nil || days <= 0 {
			a.pool.send(msg.Chat.ID, "Usage: /gencode <days>\n\nJaise: /gencode 30 — ek naya premium redeem code banata hai, jo customer ko bhejo.")
			return
		}
		code := strings.ToUpper(uuid.New().String()[:8])
		if err := a.db.createPremiumCode(ctx, code, days, msg.From.ID); err != nil {
			a.pool.send(msg.Chat.ID, "❌ Code generate nahi ho paya, dobara try karo.")
			return
		}
		base := a.cfg.baseURL()
		a.pool.sendMD(msg.Chat.ID, fmt.Sprintf(
			"✅ *Premium code ready\\!*\n\n🎟️ Code: `%s`\n⏳ Validity: %d din\n\nCustomer ko yeh code bhejo, aur unhe %s/redeem pe jaake code daalne ko bolo\\. Redeem karte hi unka current device password\\-protected sab files ke liye unlock ho jaayega\\.",
			code, days, base,
		))
	case "premium":
		if msg.From.ID != a.cfg.AdminID {
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := watchPageTmpl.Execute(w, data); err != nil {
...
func (a *App) handleRedeem(w http.ResponseWriter, r *http.Request) {
	...
	renderPage := func(message, msgClass string) {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		fmt.Fprintf(w, `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Redeem Premium Code</title>
<style>
  ...
  .card{background:#161a22;border:1px solid #2a2f3a;border-radius:16px;padding:28px;max-width:380px;width:100%%;}
...
<div class="card">
  <h1>🎟️ Redeem Premium Code</h1>
  <p>Code daalo jo tumhe diya gaya hai — is browser ke liye saari password-protected files unlock ho jaayengi.</p>
  %s
  <form method="POST">
    <input type="text" name="code" placeholder="CODE" maxlength="16" required autofocus>
	...
	if code == "" {
		renderPage(`<div class="msg err">Code daalo.</div>`, "err")
		return
	}

	expiresAt, err := a.db.redeemPremiumCode(ctx, code, deviceID)
	if err != nil {
		renderPage(`<div class="msg err">❌ Yeh code invalid hai ya already use ho chuka hai.</div>`, "err")
		return
	}
	renderPage(fmt.Sprintf(`<div class="msg ok">✅ Premium unlock ho gaya! %s tak valid hai. Ab koi bhi password-protected link direct khul jaayega.</div>`,
		expiresAt.Format("02 Jan 2006")), "ok")
}
