...
// allowOnce returns true the first time it's called for a given key within
// the ttl window, and false on any repeat call before the window expires.
// Used as a lightweight per-key throttle (e.g. rate-limiting a public form).
func (c *Cache) allowOnce(ctx context.Context, key string, ttl time.Duration) bool {
	ok, err := c.client.SetNX(ctx, "throttle:"+key, "1", ttl).Result()
	if err != nil {
		// fail-open: if redis has a hiccup, don't block legitimate users
		return true
	}
	return ok
}
...
type premiumInterestReq struct {
...
var emailLooksValidRe = regexp.MustCompile(`^[^\s@]+@[^\s@]+\.[^\s@]+$`)

// handlePremiumInterest receives the premium-plan lead form from the watch
// page and forwards it to the admin on Telegram. Nothing is stored/exposed
// beyond the admin chat — this is just a contact-me-so-I-can-sell-access flow.
func (a *App) handlePremiumInterest(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		fmt.Fprint(w, `{"ok":false,"reason":"method_not_allowed"}`)
		return
	}

	var req premiumInterestReq
	body := http.MaxBytesReader(w, r.Body, 8*1024)
	if err := json.NewDecoder(body).Decode(&req); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, `{"ok":false,"reason":"bad_json"}`)
		return
	}
	...
	req.VideoName = strings.TrimSpace(req.VideoName)

	if req.Name == "" || req.ContactID == "" || req.Email == "" {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, `{"ok":false,"reason":"missing_fields"}`)
		return
	}
	if len(req.Name) > 100 {
		req.Name = req.Name[:100]
	}
	if len(req.ContactID) > 100 {
		req.ContactID = req.ContactID[:100]
	}
	if !emailLooksValidRe.MatchString(req.Email) || len(req.Email) > 150 {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, `{"ok":false,"reason":"bad_email"}`)
		return
	}

	// simple per-IP throttle so the form can't be spammed
	ip := clientIP(r)
	throttleKey := "premium_lead:" + ip
	ctx, cancel := context.WithTimeout(r.Context(), 3*time.Second)
	allowed := a.cache.allowOnce(ctx, throttleKey, 30*time.Second)
	cancel()
	if !allowed {
		w.WriteHeader(http.StatusTooManyRequests)
		fmt.Fprint(w, `{"ok":false,"reason":"slow_down"}`)
		return
	}

	planLabel := req.Plan
	if planLabel == "" {
		planLabel = "unspecified"
	}
	var b strings.Builder
	b.WriteString("💎 *New Premium Interest*\n\n")
	fmt.Fprintf(&b, "*Plan:* %s\n", mdEscape(planLabel))
	fmt.Fprintf(&b, "*Name:* %s\n", mdEscape(req.Name))
	fmt.Fprintf(&b, "*Contact ID:* %s\n", mdEscape(req.ContactID))
	fmt.Fprintf(&b, "*Email:* %s\n", mdEscape(req.Email))
	if req.VideoName != "" {
		fmt.Fprintf(&b, "*Video:* %s\n", mdEscape(req.VideoName))
	}
	if req.VideoID != "" {
		fmt.Fprintf(&b, "*Video ID:* `%s`\n", mdEscape(req.VideoID))
	}
	fmt.Fprintf(&b, "*IP:* %s\n", mdEscape(ip))
	fmt.Fprintf(&b, "*Time:* %s\n", time.Now().Format("02 Jan 2006, 15:04 MST"))

	a.pool.sendMD(a.cfg.AdminID, b.String())

	w.WriteHeader(http.StatusOK)
	fmt.Fprint(w, `{"ok":true}`)
}

type contactAdminReq struct {
...
// handleContactAdmin lets any visitor send a free-text message straight to
// the admin's Telegram, from the "Message the Owner" widget on the watch page.
func (a *App) handleContactAdmin(w http.ResponseWriter, r *http.Request) {
	...
	if req.Message == "" {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, `{"ok":false,"reason":"empty_message"}`)
		return
	}
	...
	ip := clientIP(r)
	ctx, cancel := context.WithTimeout(r.Context(), 3*time.Second)
	allowed := a.cache.allowOnce(ctx, "contact_msg:"+ip, 15*time.Second)
	cancel()
	if !allowed {
		w.WriteHeader(http.StatusTooManyRequests)
		fmt.Fprint(w, `{"ok":false,"reason":"slow_down"}`)
	...
	var b strings.Builder
	b.WriteString("📩 *New Message from Website*\n\n")
	fmt.Fprintf(&b, "%s\n\n", mdEscape(req.Message))
	if req.VideoName != "" {
		fmt.Fprintf(&b, "*From video:* %s\n", mdEscape(req.VideoName))
	}
	if req.VideoID != "" {
		fmt.Fprintf(&b, "*Video ID:* `%s`\n", mdEscape(req.VideoID))
	}
	if req.Lang != "" {
		fmt.Fprintf(&b, "*Language used:* %s\n", mdEscape(req.Lang))
	}
...
func main() {
	...
	mux.HandleFunc("/premium-interest", app.handlePremiumInterest)
	mux.HandleFunc("/contact-admin", app.handleContactAdmin)
